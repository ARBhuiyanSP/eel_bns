...
if (isset($_POST['transfer_submit']) && !empty($_POST['transfer_submit'])) {
    ...
    */
    ...
    header("location: w_transfer_entry.php");
    exit();
	}

}

if (isset($_POST['p_transfer_submit']) && !empty($_POST['p_transfer_submit'])) {
    ...
    for ($count = 0; $count < count($_POST['quantity']); $count++) {
        ...
        */ 
        ...
        $query = "INSERT INTO `inv_tranferdetail` (`transfer_id`,`material_id`,`material_name`,`transfer_qty`,`transfer_price`,`unit`,`type`,`inproject`,`outproject`,`inwarehouse`,`outwarehouse`) VALUES ('$transfer_id','$material_id','$material_name','$quantity','$unit_price','$unit','1','$to_project','$from_project','$to_warehouse','$from_warehouse')";
        $conn->query($query);
		$lastinsertedId =  mysqli_insert_id($conn);
        ...
        */
        ...
        $mbin_out_qty      	= 0;
        $mbfrom_type		= 'Transfer In';
        $query_inmb = "INSERT INTO `inv_materialbalance` (`mb_ref_id`,`mb_materialid`,`mb_date`,`mbin_qty`,`mbin_val`,`mbout_qty`,`mbout_val`,`mbprice`,`mbtype`,`mbserial`,`mbserial_id`,`mbunit_id`,`jvno`,`project_id`, `warehouse_id`) VALUES ('$mb_ref_id','$mb_materialid','$mb_date','$mbin_in_qty','$mbin_val','$mbin_out_qty','$mbout_val','$mbprice','$mbfrom_type','$mbserial','$mbunit_id','$mbserial_id','$jvno','$to_project','$to_warehouse')";
        $conn->query($query_inmb);
		
		
		
				/* insert Qty in inv_product_price table*/
		       $queryPro = "INSERT INTO `inv_product_price`(`mrr_no`,`material_id`, `receive_details_id`, `qty`, `price`,`project_id`,`warehouse_id`, `status`) VALUES ('$transfer_id','$material_id','$lastinsertedId','$mbin_in_qty','$unit_price','$to_project','$to_warehouse','1')";
				$conn->query($queryPro);
    }
    /*
    *  Insert Data Into inv_transfermaster Table:
    */
    $query2 = "INSERT INTO `inv_transfermaster` (`transfer_id`,`transfer_date`,`no_of_material`,`totalamount`,`from_project`,`to_project`,`from_warehouse`,`to_warehouse`,`remarks`) VALUES ('$transfer_id','$transfer_date','$no_of_material','$totalamount','$from_project','$to_project','$from_warehouse','$to_warehouse','$remarks')";
    $result2 = $conn->query($query2);    
  
    
    $_SESSION['success']    =   "warehouse transfer process have been successfully completed.";
    header("location: p_transfer_entry.php");
    exit();
	}
